...
public class SphericalSurface extends ResolvableStepEntity {
    ...
    public SphericalSurface(String id,
                            String rawParameters,
                            String name,
                            String positionRef,
                            double radius) {
        super(id, StepEntityType.SPHERICAL_SURFACE, rawParameters);
        this.name = name;
    ...
    public void doResolve(StepModel model) {
        ...
        if (entity == null) {
            throw new StepResolveException(
                    "SPHERICAL_SURFACE " + getId() + " missing position reference: " + positionRef
            );
        }

        if (!(entity instanceof Axis2Placement3D axis2Placement3D)) {
            throw new StepResolveException(
                    "SPHERICAL_SURFACE " + getId() +
